...
public enum BrowseSelectUIControl {
	...
	private class SaveCartHandler implements EventHandler<ActionEvent>,Callback {		
		...
		public void doUpdate() {
			...
			try {
				controller.saveLiveCart(BrowseSelectData.INSTANCE.obtainCurrentShoppingCartSubsystem(),
									CacheReader.readLoggedIn());
				
				primaryStage.hide();
				shoppingCartWindow.show();		
				//Display that the LiveCart has been saved.
				shoppingCartWindow.displayInfo("You Current Shopping Cart is Saved.");
				
			} catch (BackendException e) {
				// TODO Auto-generated catch block
				shoppingCartWindow.displayError("Database error. Message: " + e.getMessage());
				e.printStackTrace();
			}
		...
		public void handle(ActionEvent evt) {
			...
			shoppingCartWindow.setTableAccessByRow();
			//shoppingCartWindow.setPrimaryStage(primaryStage);
			boolean isLoggedIn = CacheReader.readLoggedIn();
			
			if (!isLoggedIn) {
				LoginUIControl loginControl = new LoginUIControl(shoppingCartWindow,
						shoppingCartWindow, this);
				loginControl.startLogin();
			} else {
				doUpdate();
			}					
