...
class DbClassOrder implements IDbClassOrder {
	...
	// private DataAccessSubsystem dataAccessSS = new
	...
	// ?";
	private String orderItemsQuery = "SELECT o.*, p.productname FROM accountsdb.orderitem o join productsdb.product p where orderid = ?";
	...
	private String submitOrderQuery = "INSERT into Ord "
			...
			+ "VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	private String submitOrderItemQuery = "INSERT into orderitem (orderid,productid,quantity,totalprice,shipmentcost, taxamount)"
			+ "VALUES (?,?,?,?,?,?)";
	 ...
	 * 
	 * @see
	 * business.ordersubsystem.IDbClassOrder#getOrderItems(java.lang.Integer)
	 */
	@Override
	public List<OrderItem> getOrderItems(Integer orderId) throws DatabaseException {
		// implemented
		if (orderItems == null) {
			return refreshOrderItems(orderId);
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see business.ordersubsystem.IDbClassOrder#refreshOrderItems(java.lang.
	 * Integer)
	 */
	@Override
	public List<OrderItem> refreshOrderItems(Integer orderId) throws DatabaseException {
	...
	private void populateOrderItems(ResultSet rs) throws DatabaseException {
		...
		try {
			while (rs.next()) {
				...
				double totalPrice = rs.getInt("totalprice");
				String productname = rs.getString("productname");

				// String productName =
				// pss.getProductFromId(productId).getProductName();
	...
	private void populateOrderData(ResultSet resultSet) throws DatabaseException {
		...
		try {
			...
			while (resultSet.next()) {
				...
				orderData.setOrderId((int) orderDataParams[0]);
			}
		} catch (SQLException e) {
			throw new DatabaseException(e);
		}
	 ...
	 * 
	 * @see
	 * business.ordersubsystem.IDbClassOrder#populateEntity(java.sql.ResultSet)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see business.ordersubsystem.IDbClassOrder#getDbUrl()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see business.ordersubsystem.IDbClassOrder#getQuery()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see business.ordersubsystem.IDbClassOrder#getQueryParams()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see business.ordersubsystem.IDbClassOrder#getParamTypes()
	 */
	@Override
