...
@Repository
public class DbClassShoppingCart implements  IDbClassShoppingCart {
    ...
    private int saveCartTopLevel(CustomerProfile custProfile) throws DatabaseException {
    	...
    	saveCartParams = new Object[]{custProfile.getCustId(), cart.getShippingAddress().getStreet(),
    	    	 ...
    	    	 " ", cart.getShippingAddress().getCity(), cart.getShippingAddress().getState(), cart.getShippingAddress().getZip(), cart.getBillingAddress().getStreet(),
    	    	  
    	    	  " ",cart.getBillingAddress().getCity(), cart.getBillingAddress().getState(),cart.getBillingAddress().getZip(), cart.getPaymentInfo().getNameOnCard(), 
    	    	  
    	    	  cart.getPaymentInfo().getExpirationDate(),cart.getPaymentInfo().getCardType(),cart.getPaymentInfo().getCardNum(),cart.getTotalPrice(),0.00,
